...
import com.rahim.tunexmusic.ui.widget.TuneXWidgetProvider
...
class AudioPlaybackService : MediaSessionService() {
    ...
    private val mainPlayerListener = object : Player.Listener {
        ...
        override fun onPlayerError(error: PlaybackException) {
            ...
            Log.e(
                TAG, "ExoPlayer error: errorCode=${error.errorCode} (${error.errorCodeName ?: "?"}) " +
                    "msg=${error.message} rootCause=${rootCause?.javaClass?.simpleName ?: "none"}: ${rootCause?.message ?: ""}",
                error
            )
            viewModel?.setIsPlaying(false)

            // Surface the real error to the user instead of silently auto-advancing.
            // A failed online stream must NOT be treated as a completed track that
            // automatically moves to the next song. Stop playback and keep the current
            // song loaded so the user can retry or choose another track manually.
            val friendlyMessage = playbackErrorMessage(error)
            com.rahim.tunexmusic.ui.SnackbarController.showMessage(
                "Playback error: $friendlyMessage",
                iconResId = com.rahim.tunexmusic.R.drawable.ic_info
            )
            try {
                player.stop()
            } catch (e: Exception) {
                Log.w(TAG, "Player stop during error recovery failed: ${e.message}")
            }
            // Do NOT clear media items blindly: keep the failed item so the source URI
            // and current song remain resolvable for a retry. Also do NOT play next song.
        }
        ...
        if (uri.scheme == "innertube") {
            ...
            resolveJob = serviceScope.launch {
                ...
                if (!streamUrl.isNullOrEmpty()) {
                    ...
                    // Do NOT auto-skip: surface the real error and keep the current song.
                    viewModel?.setIsPlaying(false)
                    com.rahim.tunexmusic.ui.SnackbarController.showMessage(
                        "Couldn't load this online song. Please retry.",
                        iconResId = com.rahim.tunexmusic.R.drawable.ic_info
                    )
    ...
    private fun updateWidgetProgress(pos: Long, dur: Long) {
        ...
        viewModel?.syncedLyrics?.value?.let { lyrics ->
            if (lyrics.isNotEmpty()) {
                lyricText = TuneXWidgetProvider.findCurrentLyricLine(lyrics, pos.toInt()) ?: "..."
            }
        }
        TuneXWidgetProvider.updateAllWidgetsPartial(this, pos.toInt(), dur.toInt(), lyricText)
    }
    ...
    fun updateWidgetState(isPlaying: Boolean) {
        viewModel?.let { vm ->
            ...
            vm.syncedLyrics.value?.let { lyrics ->
                if (lyrics.isNotEmpty()) {
                    lyricText = TuneXWidgetProvider.findCurrentLyricLine(lyrics, pos) ?: "..."
                }
            }
            TuneXWidgetProvider.updateAllWidgets(this, song, isPlaying, pos, dur, lyricText)
        }
    }
    ...
    /**
     * Build a short, user-friendly message from an ExoPlayer playback error,
     * avoiding the raw stack/URL noise while keeping the actionable cause.
     */
    private fun playbackErrorMessage(error: PlaybackException): String {
        val msg = error.message?.trim().orEmpty()
        return when {
            error.errorCode == PlaybackException.ERROR_CODE_IO_UNSPECIFIED &&
                (msg.contains("403", ignoreCase = true) || msg.contains("HttpDataSource", ignoreCase = true)) ->
                "Stream blocked (HTTP 403). Please retry."
            error.errorCode == PlaybackException.ERROR_CODE_IO_NETWORK_CONNECTION_TIMEOUT ->
                "Network timeout. Please retry."
            error.errorCode == PlaybackException.ERROR_CODE_IO_BAD_HTTP_STATUS ->
                "Stream unavailable (HTTP error). Please retry."
            msg.isNotBlank() && msg.length <= 60 -> msg
            else -> "Stream could not be played. Please retry."
        }
    }
