...
package com.rahim.tunexmusic.service
...
import com.rahim.tunexmusic.R
import com.rahim.tunexmusic.data.innertube.YouTubeMusic
import com.rahim.tunexmusic.ui.widget.TuneXMusicWidgetProvider
import com.rahim.tunexmusic.util.AudioFileManager
import com.rahim.tunexmusic.util.FFmpegCommandBuilder
import com.rahim.tunexmusic.viewmodel.PlayerSharedViewModel
...
class AudioPlaybackService : MediaSessionService() {
    ...
    companion object {
        ...
        const val ACTION_PLAY = "com.rahim.tunexmusic.ACTION_PLAY"
        const val ACTION_PAUSE = "com.rahim.tunexmusic.ACTION_PAUSE"
        const val ACTION_PREVIOUS = "com.rahim.tunexmusic.ACTION_PREVIOUS"
        const val ACTION_NEXT = "com.rahim.tunexmusic.ACTION_NEXT"
        const val ACTION_TOGGLE_LOOP = "com.rahim.tunexmusic.ACTION_TOGGLE_LOOP"
        const val ACTION_CYCLE_PLAYBACK_MODE = "com.rahim.tunexmusic.ACTION_CYCLE_PLAYBACK_MODE"
        const val ACTION_TOGGLE_FAV = "com.rahim.tunexmusic.ACTION_TOGGLE_FAV"
    ...
    private var currentSongName = "TuneX Music"
    ...
    // Playback context generation. Bumped whenever the loaded song or queue
    // context changes so in-flight next-song preloads from a previous context
    // can never be injected into the newly loaded ExoPlayer timeline.
    private var queueEpoch = 0L

    private lateinit var telemetryManager: com.rahim.tunexmusic.data.innertube.YouTubeTelemetryManager
    ...
    private val mainPlayerListener = object : Player.Listener {
        ...
        override fun onPlayerError(error: PlaybackException) {
            ...
            // IMPORTANT: Do NOT auto-skip/change to another song here. The user
            // explicitly picked the current track, so silently advancing to the
            // next one is the source of the "song auto-changes" bug. Just stop
            // and keep the selected song in place so the user can retry it or
            // pick another track.
            try {
                player.stop()
            } catch (e: Exception) {
    ...
    override fun onCreate() {
        ...
        telemetryManager = com.rahim.tunexmusic.data.innertube.YouTubeTelemetryManager(
            httpClient = com.rahim.tunexmusic.data.innertube.InnerTubeClient.httpClient,
            clientProvider = { com.rahim.tunexmusic.data.innertube.InnerTubeClient }
        )
        ...
        if (uri.scheme == "innertube") {
            ...
            resolveJob = serviceScope.launch {
                ...
                if (!streamUrl.isNullOrEmpty()) {
                    ...
                    // IMPORTANT: Do NOT auto-skip to the next song here. The user
                    // tapped THIS song; silently playing another track is the
                    // reported "song auto-changes" bug. Keep the selection and
                    // just report that it is not ready.
                    viewModel?.setIsPlaying(false)
    ...
    private fun preloadNextSongIntoQueue() {
        ...
        // Capture the playback context this preload was computed under. If a
        // different song/queue is loaded before the async resolution completes,
        // this stale preload must NOT be injected into the player.
        val epochAtStart = queueEpoch
        ...
        serviceScope.launch {
            ...
            withContext(Dispatchers.Main) {
                // Discard stale preloads computed before a different song/queue
                // was loaded — injecting them changes the playing song to a
                // wrong/alternative track the user never selected.
                if (epochAtStart != queueEpoch) {
                    Log.d(TAG, "Stale next-song preload discarded (playback context changed)")
                    return@withContext
                }
    ...
    fun refreshNativeQueue() {
        // Queue content was edited — invalidate in-flight preloads so they
        // cannot inject an item that no longer reflects the current queue.
        queueEpoch++
                        ...
                        serviceScope.launch(Dispatchers.Main) {
                            viewModel?.let { vm ->
                                ...
                                val newSongs = related.map { onlineSong ->
                                    com.rahim.tunexmusic.model.SongItem.createOnlineSong(
                                        onlineSong.videoId,
                                        onlineSong.title,
                                        onlineSong.artist,
    ...
    fun pause() {
        if (player.isPlaying) {
            ...
            com.rahim.tunexmusic.util.PlaybackStateManager.savePosition(applicationContext, player.currentPosition)
        }
    }
    ...
    fun setSongMetadata(songName: String?, albumArt: Bitmap?) {
        this.currentSongName = songName ?: "TuneX Music"
    ...
    private fun loadAudioInternal(uri: Uri, keepPlayingUntilReady: Boolean = false) {
        // A new song is being explicitly loaded — invalidate any in-flight
        // next-song preload that was computed under a previous context.
        queueEpoch++
                ...
                handler.post {
                    ...
                    viewModel?.let { vm ->
                        if (vm.hapticManager == null) {
                            vm.hapticManager = com.rahim.tunexmusic.util.PlayerHapticManager(applicationContext)
                        }
                    }
                }
            } catch (ex: Exception) {
            ...
            override fun run() {
                if (player.playbackState != Player.STATE_IDLE && player.playbackState != Player.STATE_ENDED) {
                    ...
                    if (player.isPlaying && (pos / 250) % 20L == 0L) {
                        com.rahim.tunexmusic.util.PlaybackStateManager.savePosition(applicationContext, pos)
                    }
    ...
    private fun updateWidgetProgress(pos: Long, dur: Long) {
        ...
        viewModel?.syncedLyrics?.value?.let { lyrics ->
            if (lyrics.isNotEmpty()) {
                lyricText = TuneXMusicWidgetProvider.findCurrentLyricLine(lyrics, pos.toInt()) ?: "..."
            }
        }
        TuneXMusicWidgetProvider.updateAllWidgetsPartial(this, pos.toInt(), dur.toInt(), lyricText)
    }
    ...
    fun updateWidgetState(isPlaying: Boolean) {
        viewModel?.let { vm ->
            ...
            vm.syncedLyrics.value?.let { lyrics ->
                if (lyrics.isNotEmpty()) {
                    lyricText = TuneXMusicWidgetProvider.findCurrentLyricLine(lyrics, pos) ?: "..."
                }
            }
            TuneXMusicWidgetProvider.updateAllWidgets(this, song, isPlaying, pos, dur, lyricText)
        }
    }
    ...
    private fun setupMediaSession() {
        ...
        val intent = Intent(this, com.rahim.tunexmusic.MainActivity::class.java)
    ...
    private fun setupPreferenceListener() {
        prefListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            ...
            if (com.rahim.tunexmusic.ui.SettingsViewModel.KEY_VOLUME_NORMALIZATION_ENABLED == key ||
                com.rahim.tunexmusic.ui.SettingsViewModel.KEY_TARGET_LUFS == key) {
                updateBaseVolume()
            }
        }
    }
    ...
    private inner class CustomMediaNotificationProvider(context: Context) :
        ...
        override fun addNotificationActions(
            ...
            notificationBuilder.setSubText("TuneX Music")
    ...
    private fun updateBaseVolume() {
        val prefs = getSharedPreferences(com.rahim.tunexmusic.ui.SettingsViewModel.PREFS_NAME,
            MODE_PRIVATE
        )
        val enabled = prefs.getBoolean(com.rahim.tunexmusic.ui.SettingsViewModel.KEY_VOLUME_NORMALIZATION_ENABLED, false)
        ...
        val targetLufs = prefs.getFloat(com.rahim.tunexmusic.ui.SettingsViewModel.KEY_TARGET_LUFS, -14f)
        ...
        if (song.lufs != null) {
            ...
            serviceScope.launch(Dispatchers.IO) {
                val measured = com.rahim.tunexmusic.player.LoudnessAnalyzer.analyzeLufsFast(song.path)
                if (measured != null) {
                    // Update in-memory so we don't re-analyze
                    song.lufs = measured
                    // Save to playlist db if applicable
                    val db = com.rahim.tunexmusic.data.db.MusicDatabase.getDatabase(applicationContext)
            ...
            serviceScope.launch(Dispatchers.IO) {
                val loudnessDb = com.rahim.tunexmusic.data.innertube.YouTubeMusic.getLoudnessDb(song.videoId!!).getOrNull()
    ...
    private fun applyLufsGain(lufs: Float, targetLufs: Float) {
        val gain = com.rahim.tunexmusic.player.VolumeNormalizer.calculateGain(lufs, targetLufs)
        val linearVolume = com.rahim.tunexmusic.player.VolumeNormalizer.dbToLinear(gain)
