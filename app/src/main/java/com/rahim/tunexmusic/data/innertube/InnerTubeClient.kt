...
object InnerTubeClient {
    ...
    suspend fun player(videoId: String?, playlistId: String? = null): JsonObject = withContext(Dispatchers.IO) {
        ...
        // YouTube rotates its anonymous anti-bot challenge (playabilityStatus = LOGIN_REQUIRED)
        // across requests, so a single attempt frequently yields no streamingData. Retrying the
        // ANDROID client a bounded number of times materially improves the chance of observing an
        // "OK" response with playable adaptiveFormats, without changing any playback behavior.
        var androidResult: JsonObject = JsonObject()
        val maxAndroidTries = 5
        for (attempt in 1..maxAndroidTries) {
            val result = playerWithClient(
                videoId, playlistId,
            ...
            if (isPlayerResponseValid(result)) {
                Log.d(TAG, "Player success with native ANDROID core client (attempt $attempt/$maxAndroidTries)")
                return@withContext result
            }
            logPlayerFailure("ANDROID (attempt $attempt/$maxAndroidTries)", result)
            if (attempt < maxAndroidTries) {
    ...
    private suspend fun playerWithClient(
        ...
        // The Origin/X-Origin/Referer must match the Host we are posting to. If this
        // request targets www.youtube.com, YouTube rejects mismatched origins with
        // HTTP 400 "Origin doesn't match Host for XD3", which yields an empty player
        // response and breaks stream extraction. Remote music.youtube.com disallows
        // this because ANDROID/IOS clients are hosted at www.youtube.com.
        val targetYoutube = baseUrl.contains("www.youtube.com")
        val headerMap = buildHeaders(clientType, isYoutube = targetYoutube)
