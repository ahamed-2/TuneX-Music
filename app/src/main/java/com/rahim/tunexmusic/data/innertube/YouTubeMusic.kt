...
package com.rahim.tunexmusic.data.innertube
...
object YouTubeMusic {
    ...
    /**
     * Safe logging helper: reduces a stream/signed URL to its host + path only, dropping
     * all query parameters which may contain transient signatures/tokens. Never logs the
     * full URL, which can embed sensitive or noisy authorization data.
     */
    private fun urILog(url: String?): String = try {
        ...
        val activeTask = activeStreamRequests.computeIfAbsent(videoId) { id ->
            appScope.async {
                try {
                    ...
                    Log.d(TAG, "[stream] NewPipe result for $id: ${if (bestUrl.isNullOrEmpty()) "EMPTY" else "URL(${urILog(bestUrl!!)})"}")
                    ...
                    if (bestUrl.isNullOrEmpty()) {
                        ...
                        try {
                            ...
                            val playabilityStatus = playerJson.get("playabilityStatus")?.takeIf { it.isJsonObject }?.asJsonObject
                            Log.d(TAG, "[stream] InnerTube player for $id -> status=${playabilityStatus?.get("status")?.asString ?: "?"}, reason=${playabilityStatus?.get("reason")?.asString ?: "?"}, hasStreamingData=${playerJson.has("streamingData")}")
                            ...
                            Log.d(TAG, "[stream] InnerTube selected: mime=${bestStream?.mimeType}, bitrate=${bestStream?.bitrate}, urlHost=${bestStream?.let { urILog(it.url) }}")
                            if (!bestUrl.isNullOrEmpty()) {
                                Log.d(TAG, "Successfully resolved fallback stream via InnerTube player for $id")
                            }
                        } catch (fallbackEx: Exception) {
                            Log.e(TAG, "InnerTube player fallback failed for $id: ${fallbackEx.message}", fallbackEx)
                        }
                    }
    ...
    fun prefetchStream(videoId: String?) {
        if (videoId.isNullOrEmpty()) return
        com.rahim.tunexmusic.data.innertube.NewPipeStreamExtractor.prefetchBatch(
            listOf(videoId), 
            com.rahim.tunexmusic.data.innertube.NewPipeStreamExtractor.PrefetchPriority.LOW
        )
    }
     ...
     */
    fun preCacheNextSong(song: com.rahim.tunexmusic.model.SongItem) {
    ...
    @JvmStatic
    fun fetchAndAppendRelatedSongs(viewModel: com.rahim.tunexmusic.viewmodel.PlayerSharedViewModel, videoId: String) {
        kotlinx.coroutines.CoroutineScope(Dispatchers.Main).launch {
            try {
                val result = relatedSongs(videoId)
                result.onSuccess { related ->
                    related.forEach { onlineSong ->
                        val songItem = com.rahim.tunexmusic.model.SongItem.createOnlineSong(
                            onlineSong.videoId,
                            onlineSong.title,
                            onlineSong.artist,
