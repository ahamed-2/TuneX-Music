package com.rahim.tunexmusic.data.lyrics.providers

import android.util.Log
import com.rahim.tunexmusic.data.lyrics.LrcLibApi
import com.rahim.tunexmusic.data.lyrics.LrcLibResponse
    ...
    private fun findBestMatch(results: List<LrcLibResponse>, track: TrackMetadata): LyricsResult? {
        ...
        for (item in results) {
            ...
            if (track.durationMs > 0 && item.duration > 0) {
                ...
                if (diff > 60) {
                    val titleSim = com.rahim.tunexmusic.data.lyrics.LyricsNormalizer.similarity(track.cleanedTitle, item.name ?: "")
