...
import com.rahim.tunexmusic.util.CacheManager
import com.google.android.material.color.DynamicColors
import dagger.hilt.android.HiltAndroidApp
import kotlinx.coroutines.launch

@HiltAndroidApp
class TuneXMusicApplication : Application(), ImageLoaderFactory {

    companion object {
        lateinit var instance: TuneXMusicApplication
            private set
    }
    ...
    override fun onCreate() {
        ...
        kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO).launch {
            try {
                ...
                Log.d("TuneXMusicApp", "Successfully warmed OkHttp connection to YouTube CDN")
            } catch (e: Exception) {
                Log.w("TuneXMusicApp", "Failed to warm up OkHttp connection: ${e.message}")
            }

            try {
                // 2. Initialize NewPipe Extractor and perform a dummy extraction to JIT-warm the JavaScript engine
                NewPipeStreamExtractor.init(this@TuneXMusicApplication)
                ...
                Log.d("TuneXMusicApp", "Successfully JIT-warmed NewPipe JS Extractor engine")
            } catch (e: Exception) {
                Log.w("TuneXMusicApp", "Failed to warm up NewPipe Extractor: ${e.message}")
            }
        }
